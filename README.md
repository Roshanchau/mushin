...
A multi-tenant, extensible logging library for Node.js.
...
- **Pluggable adapters** — ships with a Winston adapter (console + daily-rotating files); implement `LoggerAdapter` to target any backend.

## Packages

| Package | Description |
| --- | --- |
| `@mushinn/core` | Core API — `LoggerService`, `@Loggable`, `AsyncContextService`, middleware, interfaces. |
| `@mushinn/winston` | Winston adapter — console + daily-rotating file transport. |
...
You can drive the context manually — useful for jobs, workers, or tests. This mirrors the `mushin-test` example:
...
`WinstonAdapter` logs to the console and to daily-rotating files.
...
new WinstonAdapter({
  ...
  zippedArchive: false,      // default: false (gzip rotated files)
});
```
...
| `datePattern` | `string` | `"DD-MM-YYYY"` | Rotation/date pattern for filenames. |
...
A complete, runnable example lives in [`mushin-test/`](./mushin-test/test.ts). It demonstrates:

- configuring the Winston adapter once at startup,
- `@Loggable()` on sync, async, and throwing methods,
- per-request `threadId` / `userId` isolation across sequential and concurrent requests,
- all four manual log levels.
